...
void *SafeAccess::load(void **ptr, void *default_value) {
  ...
  register void *ret asm("2"); // r2 is return value register
  asm volatile("lg %0, 0(%1)" : "=r"(ret) : "a"(ptr), "r"(default_value));
#else
  asm volatile(
      ""
      :
      : "r"(
          default_value)); // prevent compiler from optimizing the argument away
  void *ret = *ptr;
#endif
  LABEL(load_end);
  return ret;
...
#elif defined(__s390x__)
  register int32_t ret asm("2"); // r2 is return value register
  asm volatile("l %0, 0(%1)" : "=r"(ret) : "a"(ptr), "r"(default_value));
#else
  asm volatile(
      ""
      :
...
// this function skips the fault instruction pretending it has loaded
// default_value
bool SafeAccess::checkFault(StackFrame &frame) {
  ...
  frame.pc() += 2;
#elif defined(__s390x__)
  // s390x load instructions are 4 or 6 bytes
  // LG (load 64-bit) is 6 bytes, L (load 32-bit) is 4 bytes
  frame.pc() += ((pc[0] >> 8) == 0xe3) ? 6 : 4;
  frame.retval() = frame.arg1();
#else
