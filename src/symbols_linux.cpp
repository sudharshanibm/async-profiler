...
static bool isLoader(const char *image_base) { return _ld_base == image_base; }
...
private:
  ...
  bool validHeader() {
    ...
    return ident[0] == 0x7f && ident[1] == 'E' && ident[2] == 'L' &&
           ...
           ident[5] == ELFDATA_SUPPORTED && ident[6] == EV_CURRENT &&
  ...
  const char *base() { return _header->e_type == ET_EXEC ? NULL : _vaddr_diff; }
  ...
  bool loadSymbolsFromDebuginfodCache(const char *build_id,
                                      const int build_id_len);
    ...
    if (rel != NULL && relsz != 0 && relent != 0) {
      ...
      for (size_t offs = relcount * relent; offs < relsz; offs += relent) {
        ...
        if (ELF_R_TYPE(r->r_info) == R_GLOB_DAT ||
          ...
          if (sym->st_name != 0) {
            _cc->addImport((void **)(base + r->r_offset),
                           strtab + sym->st_name);
          }
        }
      }
...
const char *ElfParser::getDebuginfodCache() {
  ...
  const char *suffixes[] = {"/", "debuginfod_client/",
  ...
  for (int i = 0; i < sizeof(env_vars) / sizeof(env_vars[0]); i++) {
    ...
    if (snprintf(_debuginfod_cache_buf, sizeof(_debuginfod_cache_buf), "%s/%s",
                 env_val, suffixes[i]) < sizeof(_debuginfod_cache_buf)) {
      return _debuginfod_cache_buf;
    }
  }
...
bool ElfParser::loadSymbolsFromDebug(const char *build_id,
                                     const int build_id_len) {
  char path[PATH_MAX];
...
bool ElfParser::loadSymbolsFromDebuginfodCache(const char *build_id,
                                               const int build_id_len) {
  ...
  if (debuginfod_cache_len + build_id_len + strlen("/debuginfo") >=
      sizeof(path)) {
...
// Load symbols from the first file that exists in the following locations, in
// order, where abcdef1234 is Build ID.
...
void ElfParser::loadSymbolTable(const char *symbols, size_t total_size,
  ...
  Log::debug("loadSymbolTable: lib=%s, symbols=%p, total_size=%zu, "
             "ent_size=%zu, strings=%p",
             _cc->name(), symbols, total_size, ent_size, strings);
  const char *base = this->base();
  for (const char *symbols_end = symbols + total_size; symbols < symbols_end;
    ...
    if (sym->st_name != 0 && sym->st_value != 0) {
      ...
      if (sym->st_size != 0 || sym->st_info != 0 ||
          strings[sym->st_name] != '$') {
        _cc->add(base + sym->st_value, (int)sym->st_size,
                 strings + sym->st_name);
      }
    }
  }
...
static void collectSharedLibraries(std::unordered_map<u64, SharedLibrary> &libs,
                                   int max_count) {
...
void Symbols::parseLibraries(CodeCacheArray *array, bool kernel_symbols) {
  ...
  for (auto &it : libs) {
    ...
    CodeCache *cc = new CodeCache(lib.file, array->count(), lib.map_start,
                                  lib.map_end, lib.image_base);
    ...
    if (strchr(lib.file, ':') != NULL) {
      ...
      // Unlikely case when image base has not been found: not safe to access
      // program headers. Be careful: executable file is not always ELF, e.g.
      // classes.jsa
      ...
      UnloadProtection handle(cc);
      if (handle.isValid()) {
        ElfParser::parseProgramHeaders(cc, lib.image_base, lib.map_end,
                                       OS::isMusl());
      }
    }
  ...
  if (array->count() >= MAX_NATIVE_LIBS && !_libs_limit_reported) {
    Log::warn("Number of parsed libraries reached the limit of %d",
              MAX_NATIVE_LIBS);
...
static bool verifyBaseAddress(const CodeCache *cc, void *lib_handle) {
  ...
  if (dlinfo(lib_handle, RTLD_DI_LINKMAP, &map) != 0 ||
      dladdr(map->l_ld, &dl_info) == 0) {
    return false;
  }
...
UnloadProtection::UnloadProtection(const CodeCache *cc) {
  if (OS::isMusl() || isMainExecutable(cc->imageBase(), cc->maxAddress()) ||
      isLoader(cc->imageBase())) {
    _lib_handle = NULL;
    _valid = true;
    return;
  }

  // dlopen() can reopen previously loaded libraries even if the underlying file
  // has been deleted
  const char *stripped_name = cc->name();
  size_t name_len = strlen(stripped_name);
  if (name_len > 10 &&
      strcmp(stripped_name + name_len - 10, " (deleted)") == 0) {
